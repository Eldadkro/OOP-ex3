...
public class RGB {
	...
	public static RGB mix(RGB rgb1, RGB rgb2, double alpha) {
		RGB hue = new RGB(alpha); // creates a color that we will use to mix with
		return superpose(rgb1.filter(hue), rgb2.filter(hue.invert())); // uses given API with hue to mix
	}
